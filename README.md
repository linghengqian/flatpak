...
CLI-only session instead, start `pkttyagent --process $$ --notify-fd 1 &`. If
 ...
 polkit agent and, outside WSL (unless `FLATPAK_FORCE_TEXT_AUTH=1` is set), tells
 polkit to treat it as a fallback so any running GUI agent is preferred. When
 WSL is detected (via `WSL_INTEROP` or `WSL_DISTRO_NAME`), the text prompt is
